...
test.describe("Export PNG", () => {
  test("exporta el bloque de deuda con un enlace de descarga válido", async ({
    page,
  }) => {
    await page.addInitScript(() => {
      ...
      (
        window as {
          __exportDownloads?: Array<{ href: string; download: string }>;
        }
      ).__exportDownloads = [];
    ...
    await page.goto("/?section=deuda");

    const debtSection = page.locator("#deuda");
    const exportButton = debtSection.getByRole("button", {
      name: /Exportar PNG|Export PNG/i,
    });
    ...
    expect(exportedFile.download).toMatch(
      /^cuentas-publicas-deuda-\d{4}-\d{2}-\d{2}\.png$/,
    );
